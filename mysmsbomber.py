...
try:
  ...
  from user_agent import generate_user_agent
   ...
   exit(c.red+"Please install library\ncommand => pip install requests user_agent")
   
check_neofetch = getoutput("screenfetch")
if "not found" in check_neofetch:
        exit(c.red+"please install screenfetch \n command >  apt install screenfetch")
system("clear")     
system("screenfetch")

def banner():
   banner = (f"""
                {c.green}         [+] {c.cyan}SMS Api's: {c.green}180
                {c.green}         [+] {c.cyan}CALL Api's: {c.green}4
                 {c.green}        [+] {c.cyan}Developer: {c.green}T.me/ResQQ || T.me/LooQaat """)
   printlow(banner)

def terminal_number():
   try:
      printlow(c.purple+"\n\nSEND Your Number "+c.yellow+"[Example : +98991******] :\n"+c.re)
      number = input(c.red+" ┌─["+c.lcyan+"Sms-Bomber"+c.red+"""]
 └──╼ """+c.re+"$ "+c.pink)
      if "+98" not in number:
           exit(c.red+"\n\n[ ! ] "+c.yellow+"write number  with +98")
      return number 
...
def bombing(phone):
    ...
    while True:
      try:
        ...
        rnd_call = choice([call.ragham_call,call.paklean_call,call.novinbook_call,call.azki_call])
        Thread(target=rnd_call, args=[phone]).start()
        print(c.green+"round "+count+"finish")
        count+=1
      except Exception as e:
          pass

banner()
bombing(phone = terminal_number())